...
class SearchResultsTvAdapter : RecyclerView.Adapter<SearchResultsTvAdapter.ViewHolder>() {

    private val tvs: ArrayList<DiscoverTvListResult> = ArrayList()
    private val tvGenreList: ArrayList<Genre> = ArrayList()
    private lateinit var onSearchResultsTvClickListener: OnSearchResultsTvClickListener

    fun setData(
        tvs: List<DiscoverTvListResult>,
        tvGenreList: List<Genre>,
        onSearchResultsTvClickListener: OnSearchResultsTvClickListener
    ) {
        this.tvs.clear()
        this.tvs.addAll(tvs)
        this.tvGenreList.clear()
        this.tvGenreList.addAll(tvGenreList)
        this.onSearchResultsTvClickListener = onSearchResultsTvClickListener
    ...
    class ViewHolder(itemView: View, private val onSearchResultsTvClickListener: OnSearchResultsTvClickListener) :
        ...
        fun bind(tv: DiscoverTvListResult, genres: List<Genre>) {
            ...
            itemView.itemCardView.setOnClickListener { onSearchResultsTvClickListener.onTvItemClick(tv) }
        }
